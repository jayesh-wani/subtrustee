...
import {
  ...
  GET_SUBTRUSTEE_TRANSACTIONS_OF_SETTLEMENT,
} from "../../../Qurries";
...
function TransactionsOfSettlement() {
  ...
  useEffect(() => {
    if (
      data?.getSubtrusteeSettlementsTransactions?.cursor &&
      nextCursor !== data?.getSubtrusteeSettlementsTransactions?.cursor &&
      data?.getSubtrusteeSettlementsTransactions?.cursor !== null
    ) {
      setNextCursor(data?.getSubtrusteeSettlementsTransactions?.cursor);
    }
    if (
      data?.getSubtrusteeSettlementsTransactions?.settlements_transactions &&
      data?.getSubtrusteeSettlementsTransactions?.settlements_transactions
        .length > 0
    ) {
      setAdditionalData(
        getNormalizeObjectsArray(
          data?.getSubtrusteeSettlementsTransactions?.settlements_transactions?.map(
            (d: any) => {
              ...
              <div className="flex justify-between items-center my-4 px-4">
                ...
                <button
                  ...
                  disabled={
                    ...
                    (data?.getSubtrusteeSettlementsTransactions?.cursor ===
                      "N/A" &&
                      data?.getSubtrusteeSettlementsTransactions
